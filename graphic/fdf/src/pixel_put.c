...
/*   Updated: 2016/11/28 18:15:19 by ahoareau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_string_put(e->mlx, e->win, 600, 20, 0xFFFFFF, \
			"QUITTER = [ECHAP]   MENU = [ENTER]");
	mlx_string_put(e->mlx, e->win, 600, 50, 0xFFFFFF, "Amplitude:");
	mlx_string_put(e->mlx, e->win, 750, 50, 0xFFFFFF, ft_itoa(tab->amp));
	mlx_string_put(e->mlx, e->win, 600, 80, 0xFFFFFF, "Coordonnees:");
	mlx_string_put(e->mlx, e->win, 750, 80, 0xFFFFFF, ft_itoa(tab->pos.x));
	mlx_string_put(e->mlx, e->win, 800, 80, 0xFFFFFF, ft_itoa(tab->pos.y));
	mlx_string_put(e->mlx, e->win, 600, 110, 0xFFFFFF, "Zoom: X");
	mlx_string_put(e->mlx, e->win, 750, 110, 0xFFFFFF, ft_itoa(tab->coef));
}
